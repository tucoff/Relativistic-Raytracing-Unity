		...
		Pass
		{
			...
			float3 GetEnvironmentLight(Ray ray)
			{
				if (!EnvironmentEnabled) {
					// Fallback para evitar pontos pretos
					return float3(0.2, 0.4, 0.8); // Azul suave
				}
			...
			float3 Trace(Ray ray, inout uint rngState)
			{
				...
				for (int bounceIndex = 0; bounceIndex <= MaxBounceCount; bounceIndex ++)
				{
					...
					if (hitInfo.didHit)
					{
						...
						ray.origin = hitInfo.hitPoint + hitInfo.normal * 0.001; // Offset para evitar self-intersection
						...
						// Random early exit if ray colour is nearly 0 (com threshold mínimo)
						float p = max(rayColour.r, max(rayColour.g, rayColour.b));
						if (RandomValue(rngState) >= p && p < 0.05) {
							break;
						}
						rayColour *= 1.0f / max(p, 0.01); // Evita divisão por zero 
					}
					else
					{
